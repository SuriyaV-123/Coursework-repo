...
#the structure is the exact same as the prey's neural network
class predator_network(nn.Module):
  def __init__(self,n_actions):
    super().__init__()


    #using relu multiple times
    self.relu = nn.ReLU()
...
# Initialize weights
  ...
  def forward(self,input):
    ...
    output = torch.nan_to_num(output, nan=0.0)
    return output
#again, we are using the actor-critic network, same setup as for the prey.
...
#all of this is to make sure that there is something in the closest predator and closest food, so that when it is trying to choose an action, the program doesn't crash.
    ...
    max_x = 600
    max_y = 900
#normalising the positions to make sure that they don't get out of hand.
    current_pos_x = max(min(current_pos_x / max_x, 1), -1)
    current_pos_y = max(min(current_pos_y / max_y, 1), -1)
